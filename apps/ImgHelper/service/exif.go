...
func (ex *ExifData) GetExifData(r io.Reader) (data []byte, err error) {
	...
	if n < EXIF_HEADER_READ_SIZE {
		return nil, fmt.Errorf(fmt.Sprintf("Read short of EXIF_HEADER_READ_SIZE > %d\n", n))
	}
	...
	if bytes.Equal(b[0:4], EXIF_HEADER_SIGNATURE) &&
		...
		ex.headerLength = length - 8
		data := make([]byte, ex.headerLength)
		_, err = r.Read(data)
...
func (ex TiffData) GetIFDList() []uint32 {
	...
	ifdOffset := ex.GetFirstIFD()
	for ifdOffset != 0 {
		ifdList = append(ifdList, ifdOffset)
		ifdOffset = ex.GetNextIFD(ifdOffset)
	}
...
func (ex *TiffData) Slice(offset uint32, length uint32) *TiffData {
	ex.slice = make([]byte, length)
	ex.slice = ex.data[offset : offset+length]
	return ex
}
...
func (ex *TiffData) Uint16() uint16 {
	...
	switch ex.byteorder {
	case "M": // big-endian
		v = uint16(ex.slice[1]) | uint16(ex.slice[0])<<8
	case "I": // little-endian
		v = uint16(ex.slice[0]) | uint16(ex.slice[1])<<8
	default:
		log.Fatal(fmt.Sprintf("unrecognised byteorder: %v", ex.byteorder))
	}
...
func (ex *TiffData) Uint32() uint32 {
	...
	switch ex.byteorder {
	case "M": // big-endian
		v = uint32(ex.slice[3]) | uint32(ex.slice[2])<<8 |
			uint32(ex.slice[1])<<16 | uint32(ex.slice[0])<<24
	case "I": // little-endian
		v = uint32(ex.slice[0]) | uint32(ex.slice[1])<<8 |
			uint32(ex.slice[2])<<16 | uint32(ex.slice[3])<<24
	default:
		log.Fatal(fmt.Sprintf("unrecognised byteorder: %v", ex.byteorder))
	}
...
func (ex *TiffData) Uint64() uint64 {
	...
	case "M": // big-endian
		v = uint64(ex.slice[7]) |
			uint64(ex.slice[6])<<8 |
			uint64(ex.slice[5])<<16 |
			uint64(ex.slice[4])<<24 |
			uint64(ex.slice[3])<<32 |
			uint64(ex.slice[2])<<40 |
			uint64(ex.slice[1])<<48 |
			uint64(ex.slice[0])<<56

	case "I": // little-endian
		v = uint64(ex.slice[0]) |
			uint64(ex.slice[1])<<8 |
			uint64(ex.slice[2])<<16 |
			uint64(ex.slice[3])<<24 |
			uint64(ex.slice[4])<<32 |
			uint64(ex.slice[5])<<40 |
			uint64(ex.slice[6])<<48 |
			uint64(ex.slice[7])<<56
	default:
		log.Fatal(fmt.Sprintf("unrecognised byteorder: %v", ex.byteorder))
...
func (ex *TiffData) Int16() int16 {
	numBytes := len(ex.slice)
	msb := 2 << ((uint8(numBytes) * 8) - 1) // shift 1 to msb
	v := ex.Uint16()                      // unsigned value
...
func (ex TiffData) ProcessIFD(ifdSeq uint8, ifdOffset uint32, tagDict map[uint16]string) []IfdEntries {
	entries := ex.Slice(ifdOffset, 2).Uint16()
	...
	for i := uint16(0); i < entries; i++ {
		...
		tag := ex.Slice(base, 2).Uint16() // tag: 2 bytes
		format := ex.Slice(base+2, 2).Uint16()
		count := ex.Slice(base+4, 4).Uint32()
		offset := base + 8 // tag + format + count bytes
		formatLen := uint32(formatBytes[format-1])

		if count*formatLen > 4 { // have offset, not value
			offset = ex.Slice(offset, 4).Uint32()
		}

		var values []interface{}

		if format == 2 { // string NULL terminated
			v := ex.Slice(offset, count-1).slice
			values = append(values, fmt.Sprintf("%s", v))
		} else {
			for i := uint32(0); i < count; i++ {
				if format%5 != 0 { // non-rationals
					vptr := ex.Slice(offset, formatLen)
					...
					num := ex.Slice(offset, 4).Int32()
					den := ex.Slice(offset+4, 4).Int32()
...
func (ex *ExifData) GetTagValues(tag uint16) (values interface{}, ok bool) {
	for _, entries := range ex.IfdData {
...
func (ex *ExifData) ProcessExifStream(f multipart.File) (err error){
	...
	for k, v := range ifdList {
		...
		ex.IfdData[IfdSeqMap[uint8(k)]] = ifds
		...
		if gpsInfoOffset != 0 {
			...
			ex.IfdData[IfdSeqMap[3]] = gpsInfo
		}

		if iopOffset != 0 {
			iopInfo := tiff.ProcessIFD(4, iopOffset, EXIF_IOP_TAGS)
			ex.IfdData[IfdSeqMap[4]] = iopInfo
		}
	}
	return
}